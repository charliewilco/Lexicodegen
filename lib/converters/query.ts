...
export async function convertQuery(
	...
	const _possibleErrors = new Set<string>();

	_possibleErrors.add("InvalidRequest"); // assuming it's always possible
	_possibleErrors.add("ExpiredToken");
	_possibleErrors.add("InvalidToken");

	if (query.errors) {
		for (const { name } of query.errors) {
			_possibleErrors.add(name);
		}
	}
			...
			"application/json": {
				schema: {
					...
					properties: {
						error: {
							type: "string",
							enum: Array.from(_possibleErrors),
						},
						message: {
							type: "string",
						},
